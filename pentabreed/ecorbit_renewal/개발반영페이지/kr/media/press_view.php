<?
	...
	if ($idx == "")	$fg->htmlBack("상세정보가 부족합니다.");
	...
	if ($ResultSet == null)
	{
		$fg->htmlBack("등록되지 않은 보도자료 게시물 정보입니다. 확인 후, 다시 입력해주시기 바랍니다.");
	}
	else
	{
        ...
        <main class="media news_detail">
            ...
            <section class="content_area">
                <article class="news_detail_area w1360">
                   <div class="news_detail_header_wrap">
                    <div class="news_detail_header_area">
                        <div class="small_header"><?=codeTonNameTag($m_hashTag, $LangCode)?><span class="small_header_date"><?=$m_pubDT?></span></div>
                ...
                <div class="com_btn_area news_detail_button">
                    <button type="button" class="btn" onclick="location='<?=$G_KO_FOLDER?>/media/press?page=<?=$NowPage?>'">목록</button>
                </div>
                
				<?	if ($relayCount > 0) { ?>
                <article class="related_news_area">
                    <div class="related_news_wrap w1360">
                        <span class="related_news_header sub_title_02 black">관련 뉴스</span>
                        <ul class="related_news_list clrfix">
							...
							<li class="related_news_list_item" data-src-tag="rnd mna">
								<a href="<?=$G_KO_FOLDER?>/media/press_view?<?=$categoryURL?>idx=<?=trim($list["idx"])?>&page=<?=$NowPage?>">
