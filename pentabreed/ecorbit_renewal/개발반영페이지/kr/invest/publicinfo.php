<?
	...
	$PAGECODE = "KR_INVEST_21";

	$tblname	= "tbl_invest_public_info";
	$codeTbl	= "tbl_common_code";
	$FullDir	= "/upload/publicInfo";
	$LangCode	= "KR";
                ...
                <div class="meun_list_wrap">
                    <div class="main_tab_area">
                        <div class="sub_tab_wrap">
                            <ul class="sub_tab">
                                <li class="green current on">
									<a class="1depth_category" data-business-area="ecorbit" href="javascript:void(0);">에코비트</a>
                                ...
                                <li class="blue">
									<a data-business-area="water" class="1depth_category" href="javascript:void(0);">워터</a>
                                    <?	if ($TotalCnt_Water > 0) { ?>
									<ul class="sub_meun_list">
										<?
                                ...
                                <li class="orange">
									<a data-business-area="energy" class="1depth_category" href="javascript:void(0);">에너지</a>
                                ...
                                <li class="lightgreen">
									<a class="1depth_category" data-business-area="green" href="javascript:void(0);">그린</a>
                                    <?	if ($TotalCnt_Green > 0) { ?>
									<ul class="sub_meun_list">
									<?
                                ...
                                <li class="purple">
									<a class="1depth_category" data-business-area="future" href="javascript:void(0);">미래사업</a>
						...
						<form name="form" method="post" action="/kr/invest/publicinfo">
                        <div class="menu_search_bar">
                            <button class="button_open" type="button">검색</button>
                            <div class="search_input">
                                <input type="text" id="id_w" name="w" value="<?=$word?>" placeholder="검색어를 입력해주세요 ">
                ...
                <article class="download_item_area">
                    <div class="box_list_area w1440">
						<?
							...
							$query = "SELECT idx, sPrtDT, sMainCg, sSubCg, sTitle, sWriter, sReceiptDT, sDartURL, sFileName, sSaveName, iFileSize, sThumbFileName, sThumbSaveName, iThumbFileSize FROM ". $tblname ." WHERE ". $whereCause ." ORDER BY iOrderNo DESC";
							$result	= $fd->query($query);
							while ($list = $fd->fetchArray($result)) {
						...
						?>
                        <div class="box_area <?=$areaColor?>" data-business-area="<?=$area?>" data-subsidiary-company="<?=trim($list["sSubCg"])?>">
                            <a href="<?=trim($list["sDartURL"])?>" target="_blank" class="link">
                                <span class="number"><?=$NowNo?></span>
                                <span class="box_title"><?=stripslashes(trim($list["sTitle"]))?></span>
                                <span class="etc_text"><?=trim($list["sReceiptDT"])?><br><?=trim($list["sWriter"])?></span>
                            </a>
                        </div>
						<?
								$NowNo--;
