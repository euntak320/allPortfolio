	...
	$(function(){
		...
		$.ajax({
	        ...
	        type: 'POST',
	        data: "htCode="+$("#hos_idx option:selected").val(),
	        async : false,
	        error: function(output) {
	        	alert("오류가 발생하였습니다. 관리자에게 문의하세요.");
	        },
		...
		$(".btnNext").click(function(){
			...
			if($("#signup_step").val() == "1"){
				...
				$.ajax({
			        type: 'POST',
			        url: '/doctor/mypage/mypage_proc.json',
			        data: frmData,
			        async : false,
			        ...
			        success: function(data) {
			        	if(data.result == 'success'){
			        		alert("의사정보를 수정하였습니다.");
			        		location.href="/doctor/mypage/mypage.do";
						} else if(data.result == 'fail') {
							alert(data.msg);
							return;
						} else if(data.result == 'alterloc') {
							alert(data.msg);
							location.href=data.locurl;
							return;
						} else {
							alert("오류가 발생하였습니다.");
							return;
