...
var gridTotalSize; // 전체데이터건수

// 주어진 페이지 번호에 따라 페이지 네비게이션 html을 만들고 gridPageNavigationDiv에 innerHTML로 넣어줍니다.
function drawGridPagingNavigation(goPage, rowsPerPage, totalPage, totalsize) {
			
	gridCurrentPage = goPage;
	gridRowsPerPage = rowsPerPage;	// 1페이지에서 보여줄 행 수 	
 	gridTotalPage = totalPage;	// 전체 페이지 계산
 	gridTotalSize = totalsize; // 전체데이터건수	
 	//console.log(gridCurrentPage, gridRowsPerPage,gridTotalPage,gridTotalSize )
 	
 	var info = gridTotalSize + '건[' + gridCurrentPage + '/' + gridTotalPage + ' 페이지]';
	...
	retStr += "<ul class='pagination'>";
	
	// 맨앞으로
	if (goPage > 1) {		
		retStr += "<li class='ico'><a href=\"javascript:movePage('1')\"><i class='fa fa-angle-double-left fa-lg'></i></a></li>";
	} 
	// 앞으로	
	if (goPage > gridViewPageCount) {
		retStr += "<li class='ico'><a href=\"javascript:movePage('" + prepage + "')\"><i class='fa fa-angle-left fa-lg'></i></a></li>";		
	}

	for (var i = (1 + prepage); i < gridViewPageCount + 1 + prepage; i++) {
		if (goPage == i) {
			retStr +="<li><a href=\"javascript:;\"><strong>" + i + "</strong></a></li>";			
		} else {
			retStr += "<li><a href=\"javascript:movePage('" + i + "')\">" + i + "</a></li>";			
		}
	...
	if (nextpage <= gridTotalPage) {
		retStr += "<li class='ico'><a href=\"javascript:movePage('" + nextpage + "')\"><i class='fa fa-angle-right fa-lg'></i></a></li>";		
	}
	// 맨뒤로	
	if (goPage != gridTotalPage) {
		retStr += "<li class='ico'><a href=\"javascript:movePage('" + gridTotalPage + "')\"><i class='fa fa-angle-double-right fa-lg'></i></a></li>";		
	}
...
//\ud30c\uc77c\uc758 \ud655\uc7a5\uc790\ub97c \uac00\uc838\uc62e
	...
	// 첨부파일 사이즈 체크 함수
	function checkAddFile(obj, fileType, fileUnit, maxSize ){
		if($(obj).val() != "") {
			// 파일 타입별 체크 (이미지, 문서, 모두)
			var extType1 = ["gif","jpg","jpeg","png"];
			var extType2 = ["hwp","pdf","doc","xls","xlsx","ppt","pptx","docx"];
			var extType3 = ["zip","alz","rar","7z"];
			
			var fileExt = $(obj).val().split(".").pop().toLowerCase();
			
			var isProc = true;
			
			if(fileType == "img") {
				if($.inArray(fileExt, extType1) == -1){
					alert("이미지 파일 [gif, jpg, jpeg, png]만 첨부 가능합니다.");
					
					isProc = false;
				}
			} else if(fileType == "all") {
				if($.inArray(fileExt, extType1) == -1 && $.inArray(fileExt, extType2) == -1 && $.inArray(fileExt, extType3) == -1){
					alert("이미지 파일, 문서파일, 압축파일만 첨부 가능합니다.");
			...
			if(isProc) {
				...
				if(fileUnit == "kb") {
					fileMaxSize = 1024 * maxSize;
					
					if(fileSize > fileMaxSize) {
						msg = "이미지파일 용량을 " + maxSize + "kb 이하로 줄여 주세요.";
						msg += "\n※ 조정방법 : 알씨 프로그램 설치 된 경우(업무용PC) 해당 그림파일 더블 클릭 -> 이미지 파일 열린 상태서 Ctrl+R -> 용량으로 조절하기 클릭 -> 1000KB 변경 -> 확인";
					...
					fileMaxSize = 1024 * 1024 *maxSize;
					
					if(fileSize > fileMaxSize) {
						if($.inArray(fileExt, extType1) > -1) {
							msg = "이미지파일 용량을  " + maxSize + "mb 이하로 줄여 주세요.";
							msg += "\n※ 조정방법 : 알씨 프로그램 설치 된 경우(업무용PC) 해당 그림파일 더블 클릭 -> 이미지 파일 열린 상태서 Ctrl+R -> 용량으로 조절하기 클릭 -> 1000KB 이하로 변경 -> 확인";
							
							alert(msg);
						} else if($.inArray(fileExt, extType2) > -1 || $.inArray(fileExt, extType3) > -1) {
							msg = "문서 또는 압축파일 용량을 " + maxSize + "mb 이하로 줄여 주세요.";
							msg += "\n[문서인 경우 문서내의 이미지 파일 용량을 줄여 주세요.]";
							msg += "\n※ 조정방법 : 알씨 프로그램 설치 된 경우(업무용PC) 해당 그림파일 더블 클릭 -> 이미지 파일 열린 상태서 Ctrl+R -> 용량으로 조절하기 클릭 -> 1000KB 이하로 변경 -> 확인";
